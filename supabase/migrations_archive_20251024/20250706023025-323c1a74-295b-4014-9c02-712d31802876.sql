...
-- Create a consultant profile using the authenticated user (handle duplicates)
INSERT INTO public.consultants (id, user_id, tier, bio, calendar_link, is_active) 
VALUES (gen_random_uuid(), '952c1a39-f9bf-4f5d-ba81-fac0ab686384', 'platinum', 'Senior Strategy Consultant with 15+ years helping Fortune 500 companies scale operations', 'https://calendly.com/demo-consultant', true)
ON CONFLICT (user_id) DO UPDATE SET
  tier = 'platinum',
  bio = 'Senior Strategy Consultant with 15+ years helping Fortune 500 companies scale operations',
  calendar_link = 'https://calendly.com/demo-consultant',
  is_active = true;

-- Delete existing services for this consultant to avoid duplicates
DELETE FROM public.services WHERE consultant_id = (SELECT id FROM public.consultants WHERE user_id = '952c1a39-f9bf-4f5d-ba81-fac0ab686384' LIMIT 1);
